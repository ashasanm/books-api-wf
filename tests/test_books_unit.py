...
The DynamoDB repository layer is fully mocked so these tests run without
...
class TestCreateBook:
    def test_create_book_success_returns_201(self):
        with patch("app.routers.books.repository.create_book") as mock_create:
            mock_create.return_value = None
            response = client.post("/api/books", json=VALID_BOOK)

        assert response.status_code == 201

    def test_create_book_missing_field_returns_400(self):
        """FastAPI / Pydantic validation should reject incomplete payloads."""
        incomplete = {k: v for k, v in VALID_BOOK.items() if k != "serial"}
        response = client.post("/api/books", json=incomplete)

        assert response.status_code == 422  # Pydantic validation error
        body = response.json()
        assert "serial" in str(body)
    ...
    def test_create_book_extra_fields_ignored(self):
        """Extra JSON keys should not cause a failure."""
...
class TestGetBook:
    def test_get_book_success_returns_200_with_body(self):
        with patch("app.routers.books.repository.get_book") as mock_get:
    ...
    def test_get_book_response_schema_matches_spec(self):
        """All five fields must be present in the response body."""
        with patch("app.routers.books.repository.get_book") as mock_get:
            mock_get.return_value = VALID_BOOK.copy()
            response = client.get("/api/books/id1")
